...
@objc open class Delaunay : NSObject {
    ...
    static var layerTriangles: Array<Triangle> = [Triangle]()
    ...
    private func updateTriangle(tr: Triangle, vertices: [Vertex]) -> Triangle {
        let updatedTriangle = tr

        updatedTriangle.vertex1 = vertices[tr.vertex1.identifier]
        updatedTriangle.vertex2 = vertices[tr.vertex2.identifier]
        updatedTriangle.vertex3 = vertices[tr.vertex3.identifier]

        return updatedTriangle
    }

    open func triangulate(_ vertices: [Vertex]) -> [Triangle] {
        if !Delaunay.layerTriangles.isEmpty {
            return Delaunay.layerTriangles.map { return updateTriangle(tr: $0, vertices: vertices) }
        }
        ...
        /* Yay, we're done! */
        Delaunay.layerTriangles = results
        return results
    }
}
